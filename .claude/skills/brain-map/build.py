#!/usr/bin/env python3
"""Brain Studio — turn any markdown vault into an interactive knowledge galaxy.
...
def preview_of(path):
    ...
    if text.startswith("---"):
        ...
        if m:
            d = re.sub(r"\s+", " ", m.group(1).strip().strip('"').strip("'"))
            return (d[:700] + "…") if len(d) > 700 else d
...
def build(vault, out, config):
    ...
    brand = {"name": config.get("name", "AI WORKSHOP OS"), "logo": False}

    os.makedirs(out, exist_ok=True)
    stamp = int(time.time())
    for fname in ("index.html", "3d.html"):
